## Good read on Decoraters and Properties in Python

https://www.programiz.com/python-programming/property

https://www.programiz.com/python-programming/decorator

## Great Article on Python Mock Module and Patching Functions

https://kirankoduru.github.io/python/mocking-with-python-part-1.html

https://kirankoduru.github.io/python/mocking-with-python-part-2.html

## Mock and Patching of Builtin functions for consistent output or different return values when called multiple times.

https://andressa.dev/2019-07-20-using-pach-to-test-inputs/

## Parameterized multiple (test input) using pytest
Refer to fizzbuzz.py and test_fizzbuzz.py files
